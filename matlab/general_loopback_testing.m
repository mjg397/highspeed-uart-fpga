...
%% ===== USER SETTINGS =====
portName = "COM9";
baudRate = 12000000;      % set to whatever you're testing
N = 256;                % bytes per trial
trials = 100;           % number of repetitions
timeout_s = 2;          % per-trial timeout
...
%% ===== RUN TESTS =====
fprintf("Running %d trials on %s @ %d baud, N=%d bytes...\n\n", trials, portName, baudRate, N);

for k = 1:trials
    tStart = tic;

    % Open port fresh each trial (most robust)
    ...
    if numel(rx) == N
        ...
        if ~same
            % Find first mismatch (0-based index for convenience)
...
if ~isempty(badIdx)
    ...
    % show top 10 most common mismatch indices
    [u,~,ic] = unique(badIdx);
